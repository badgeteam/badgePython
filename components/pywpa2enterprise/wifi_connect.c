...
bool wifi_connect_to_stored() {
    ...
    uint8_t authmode_u8;
    res = nvs_get_u8(handle, "wifi.authmode", &authmode_u8);
    authmode = (wifi_auth_mode_t) authmode_u8;
    ...
    if (use_ent) {
        ...
        // Read phase2 mode.
        uint8_t phase2_u8;
        res = nvs_get_u8(handle, "wifi.phase2", &phase2_u8);
        phase2 = (esp_eap_ttls_phase2_types) phase2_u8;
        if (res) goto errcheck;
