...
import machine, gc, time, uos, json, sys, system, virtualtimers, wifi, term, term_menu, orientation, display, buttons, nvs
...
cfg_term_menu = nvs.nvs_get_u8('splash', 'term_menu') # Show a menu on the serial port instead of a prompt
if cfg_term_menu == None:
	cfg_term_menu = True # If not set the menu is shown

cfg_wifi = nvs.nvs_get_u8('splash', 'wifi') # Allow the use of WiFi on the splash screen
if cfg_wifi == None:
	cfg_wifi = False # If not set the use of WiFi is not allowed

cfg_services = nvs.nvs_get_u8('splash', 'services') # Enable splash screen services (fun but dangerous)
if cfg_services == None:
	cfg_services = False # If not set services are disabled

cfg_logo = nvs.nvs_getstr('splash', 'logo') # Filename of a PNG image to show on the splash screen

cfg_nickname = nvs.nvs_get_u8('splash', 'nickname') # Show the nickname of the user on the splash screen
if cfg_nickname == None:
	cfg_nickname = True # If not set we want to show the nickname

cfg_greyscale = nvs.nvs_get_u8('splash', 'greyscale') # Use greyscale mode
if cfg_greyscale == None:
	cfg_greyscale = False # Disabled by default

cfg_led_animation = nvs.nvs_getstr('splash', 'ledApp') # Application which shows a LED animation while the splash screen is visible

cfg_nick_text = nvs.nvs_getstr("owner", "name")
...
def drawTask(onSleep=False):
	...
	if gui_redraw or onSleep:
		...
		if onSleep:
			info = 'Sleeping...'
			owner=nvs.nvs_getstr("owner", "name")
...
def scrollerTask():
	global scrollerStartPos, scrollerEndPos, scrollerPos
...
if cfg_term_menu:
	umenu = term_menu.UartMenu(cbSleep, None, False)
	umenu.main()
else:
	print("Welcome!")
