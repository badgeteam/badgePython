...
int32_t pin = CONFIG_NEOPIXEL_PIN;
...
esp_err_t driver_neopixel_enable(int gpio_pin)
{
	if (pin != gpio_pin) {
		driver_neopixel_disable();
	}
	...
	config.gpio_num = gpio_pin == -1 ? CONFIG_NEOPIXEL_PIN : gpio_pin;
...
esp_err_t driver_neopixel_send_data(uint8_t *data, int len)
{
	if (!driver_neopixel_active) { //return ESP_FAIL;
		esp_err_t res = driver_neopixel_enable(CONFIG_NEOPIXEL_PIN); //For backwards compatbibility: enable if not enabled already
		if (res != ESP_OK) return res;
	}
